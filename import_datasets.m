...
It also deletes extra timestamp columns and any columns with values of 0.
%}
...
%           fprintf('\nHere is column #%d, which is called %s\n', col, dataset.Properties.VariableNames{col})
          ...
          if avg > 1000
              if previous_is_timestamp
                  % fprintf('\nNeed to delete column #%d, which is called %s\n', col, dataset.Properties.VariableNames{col})
                  % fprintf('Average is %d\n', avg)
                  columnsToDelete(end+1) = dataset.Properties.VariableNames{col};
              end
              previous_is_timestamp = true;
          % should delete column if average is 0
          elseif avg == 0
              % fprintf('\nNeed to delete column #%d, which is called %s\n', col, dataset.Properties.VariableNames{col})
              % fprintf('Average is %d\n', avg)
              columnsToDelete(end+1) = dataset.Properties.VariableNames{col};
          else
              % if avg is normal then restart the flag and do not delete
              % the column
              previous_is_timestamp = false;
          end
      end
